...
ball_body_eval = plant.GetBodyByName("body_link", model_instance_ball)
...
X_WOball_initial = plant.EvalBodyPoseInWorld(temp_plant_context, ball_body_eval)
...
def ball_across_y(p_WO: np.ndarray) -> bool:
    """
...
while simulator.get_context().get_time() < T_final:
    ...
    if ball_across_y(p_WO) and landed_pos is None:
        ...
        # Optional: break if you want to stop here
        # break
...
if landed_pos is None:
    print("Ball never entered the bin interior.")
else:
...
print("END-TO-END ERROR", calc_e_to_e_error(np.array([0, -2.25, 0]), landed_pos))
